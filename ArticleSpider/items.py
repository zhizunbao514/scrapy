...
def data_conver(value):
    value = value.strip().replace("·", "").strip()
    # logging.debug('This is debug message',value)
    try:
...
def gen_suggests(index, info_tuple):
    ...
    for text, weight in info_tuple:
        if text:
            ...
            #如果没有text new_words是空的
            new_words=set()

        if new_words:
            #将新词添加到已经分析过的set里（update是添加多项）
...
class JobBoleArticleItem(scrapy.Item):
    ...
    create_date = scrapy.Field(
        input_processor=MapCompose(data_conver)
    )
    url = scrapy.Field()
    ...
    def get_insert_sql(self):
        ...
        insert_sql = """
            insert into zhihu_answer(zhihu_id,url,question_id,author_id,content,parise_num,comments_num,
              creat_time,update_time,crawl_time
            ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            ON DUPLICATE KEY UPDATE content=VALUES (content),comments_num=VALUES(comments_num), parise_num=VALUES(parise_num),
              update_time=VALUES (update_time)
        """

        create_time = datetime.datetime.fromtimestamp(self["creat_time"]).strptime(SQL_DATETIME_FORMAT)
...
def handle_jobaddr(value):
    ...
    addr_list = [item.strip() for item in addr_list if item.strip() != "查看地图" and item.strip()]
